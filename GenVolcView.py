...
class DataFile:
    ...
    def __init__(self, data_file, file_date, sectors):
        ...
        # make sure all required bands/types have been created
        # Run this in a seperate process so it doesn't break PySide2
    ...
    def check_api(self):
        ...
        try:
            res = requests.get(self._anc_request_url + "bandApi/all", headers=headers)
        except requests.exceptions.ConnectionError:
            logging.warning("Unable to connect to server to check bands. Assuming good...")
            bands = required_bands
        else:
            bands = [band['band'] for band in res.json()]

        try:
            res = requests.get(self._anc_request_url + "dataTypeApi/all", headers=headers)
        except requests.exceptions.ConnectionError:
            logging.warning("Unable to connect to server to check data Types. Assuming good...")
            data_types = required_types
        else:
            data_types = [x['dataType'] for x in res.json()]

        headers = {'username': 'internalavo',
                   'password': 'volcs4avo', }
        ...
        for dtype in required_types:
            if dtype.lower() not in data_types:
                ...
                request = {'label': dtype,
                           'dataType': dtype.lower(), }
                res = requests.post(self._anc_request_url + 'dataTypeApi/dataType',
                                    data=json.dumps(request),
                                    headers=headers)
                print(res.status_code, res.text)
        ...
        for band in required_bands:
            if band not in bands:
                print(f"Missing required band {band}. Adding.")
                color_scheme = 'DK' if band in ('LowTrop', 'MidTrop') else 'LT'
                request = {'label': band,
                           'band': band,
                           'colorSchemeCd': color_scheme, }

                res = requests.post(self._anc_request_url + 'bandApi/band',
                                    data=json.dumps(request),
                                    headers=headers)
                print(res.status_code, res.text)
    ...
    def process_data(self):
        ...
        for idx, height in enumerate(self._heights):
            ...
            try:
                ...
                logging.warning("No data found for %s, %s", self._file_name, height)
                continue
    ...
    def _run_processes(self, *args, **kwargs):
        ...
        for sector in self._sectors:
            ...
            if len(sector_processes) >= mp.cpu_count():
                while True:
                    sector_processes[0].join(.5)  # wait for the first one to be done
                    if not sector_process[0].is_alive():
                        break
            ...
            if DEBUG:
                ...
                if self.use_spawn:
                    ...
                    sector_process = self._mpctx.Process(target=self._generate_sector,
                                                         args=(sector, ) + args,
                                                         kwargs=kwargs)
                else:
                ...
                sector_process.daemon = True
                sector_processes.append(sector_process)
                sector_process.start()
                logging.debug("Process Status: %r", sector_process.is_alive())

        for idx, proc in enumerate(sector_processes):
            logging.debug("Waiting for process %i to complete", idx)
            for cnt in range(12):
                proc.join(5)  # Wait for all sectors to be complete, but no more than two minutes
                if not proc.is_alive():
                    break
                logging.debug("Still waiting for process after %i, status: %r", cnt, proc.is_alive())
            else:
                logging.debug("Killing process after wait.")
                proc.terminate()
    ...
    def _apply_filter(self, filter_, data_source = None):
        ...
        for item in data_source:
            try:
                if data_source[item].shape[0] == filter_.shape[0]:
                    data[item] = data_source[item][filter_]
                else:
                    data[item] = data_source[item][:]
            except Exception:
                continue
    ...
    def _generate_sector(self, sector, band=None, gen_cloud=False):
        ...
        if not sector_data['latitude'].any():
            ...
            logging.debug("No data found for %s", band)
            return
        ...
        # everything else
        sector_data['x_laea'] = x_laea
        sector_data['y_laea'] = y_laea
        ...
        def plot_dataset(dataset, color_map, scale_labels):
            ...
            if covered_percent > .17:
                ...
                # if DEBUG:
                if DEBUG or sector['sector'] == '1kmHIKI':
                    if DEBUG:
                        print(f"^^^^GENERATING IMAGE FOR FILE^^^")
                    dest_dir = f"/data/VolcViewImages/{sector['sector']}"
                ...
                print("Not enough coverage to bother with")
    ...
    def _volcview_upload(self, img, sector, band):
        ...
        while True:
            try:
                img.seek(0)
                res = requests.post(self._request_url + self._upload_path, files=files,
                                    data=request_data, headers=request_headers)
                break
            except Exception:
                print("Upload Failure. Waiting 5 seconds to retry")
                time.sleep(5)

        print("Lower 48", sector['name'], filename, res.status_code)
        status_1 = res.status_code

        if self._anc_request_url:
            while True:
                try:
                    img.seek(0)
                    res = requests.post(self._anc_request_url + self._upload_path, files=files,
                                        data=request_data, headers=request_headers)
                    break
                except Exception:
                    print("Upload Failure. Waiting 5 seconds to retry")
            ...
            print("Anchorage", sector['name'], filename, res.status_code)
            status_2 = res.status_code
        else:
            status_2 = 200  # If no anc URL, call it good
        ...
        if status_1 == status_2 == 200:
            ...
            SQL = """
            INSERT INTO vvstatus (sector,last_update)
            VALUES (%s,%s)
...
def main(data_file, data_date, use_spawn=True):
    ...
    file_processor = DataFile(data_file, data_date, volcview_sectors)
...
if __name__ == "__main__":
    ...
    DIR = '/data/tropomi_data/Offline/Kilauea'

    for item in os.listdir(DIR):
        if not item.endswith('.nc'):
            continue

        file_date = parse(item.split('____')[1].split('_')[0] + "Z")
        file = os.path.join(DIR, item)
        main(file, file_date)

    exit()

    FILE = '/Volumes/Transfer/TROPOMIData/NRTI/2020-01-19/S5P_NRTI_L2__SO2____20200115T010108_20200115T010608_11685_01_010107_20200115T014147.nc'

    FILE_DATE = datetime(2020, 1, 15, 1, 1, 8, tzinfo=pytz.UTC)
