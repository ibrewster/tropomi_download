...
class DataFile:
    ...
    def __init__(self, data_file, sectors = config.VOLCVIEW_SECTORS):
        ...
        du_keys = [0, 0.05, 0.1, 0.25, 0.6, 1]
        scale_diff = self._scale_stop - self._scale_start
        
        self._du_scale_labels = {
            k: f"{self._scale_start + (k * scale_diff):.1f} DU" if k < 1 else f">{self._scale_stop:.1f} DU"
            for k in du_keys
        }        
#        self._du_scale_labels = {0: "0 DU", .05: "1 DU", .1: "2 DU", .25: "5 DU", .6: "12 DU", 1: ">20 DU", }
    ...
    def _plot_altitude(self, dataset, band, sector):
        ...
        if band != 'cloud':
            ...
            dataset -= self._scale_start
            dataset *= (1 / (self._scale_stop - self._scale_start))
            dataset[dataset > 1] = 1
            dataset[dataset < 0] = 0
